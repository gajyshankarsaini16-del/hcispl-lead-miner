...
import { listSearchHistory } from "@/lib/repo";
import { ScoreBar } from "@/components/ScoreBar";
import { getCompanyById } from "@/lib/repo";

export default async function HistoryPage() {
  const history = listSearchHistory(100) as Array<{
    id: number;
    query: string;
    query_type: string;
        ...
        {history.length === 0 ? (
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-line">
              {history.map((r) => {
                const company = r.company_id ? getCompanyById(r.company_id) : undefined;
                return (
                  <tr key={r.id}>
                    ...
                    <td className="px-5 py-3">
                      {company ? <ScoreBar score={company.lead_score} /> : <span className="text-text-muted">—</span>}
                    </td>
