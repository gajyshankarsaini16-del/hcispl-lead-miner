...
export async function POST(req: NextRequest) {
  ...
  await run("INSERT INTO api_keys (user_id, label, key_prefix, key_hash) VALUES ($1, $2, $3, $4)", [
    session.userId,
    parsed.data.label,
    prefix,
    keyHash,
...
export async function DELETE(req: NextRequest) {
  ...
  await run("DELETE FROM api_keys WHERE id = $1 AND user_id = $2", [Number(id), session.userId]);
