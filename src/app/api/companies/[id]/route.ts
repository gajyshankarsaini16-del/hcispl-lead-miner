...
export default async function CompanyProfilePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  >
    <Globe size={13} /> {company.website.replace(/^https?:\/\//, "")}
      ...
      {company.summary && (
        <p className="mt-5 max-w-2xl text-sm text-text-muted leading-relaxed border-l-2 border-brass/50 pl-4">
          {company.summary}
        </p>
      )}
                  ...
                  <tr key={c.id}>
                    ...
                    <td className="px-5 py-3">
                      <div className="flex items-center gap-3 text-text-muted">
                        {c.business_email && (
                          <a href={`mailto:${c.business_email}`} title={c.business_email} className="hover:text-brass-strong">
                            <Mail size={14} />
                          </a>
                        )}
                        {c.business_phone && (
                          <a href={`tel:${c.business_phone}`} title={c.business_phone} className="hover:text-brass-strong">
                            <Phone size={14} />
                          </a>
                        )}
                        {c.linkedin && (
                          <a href={c.linkedin} target="_blank" rel="noopener noreferrer" className="hover:text-brass-strong">
                            <Link2 size={14} />
                          </a>
                        )}
                        {!c.business_email && !c.business_phone && !c.linkedin && "—"}
                      </div>
                    </td>
