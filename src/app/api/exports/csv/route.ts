...
import { listCompanies, getContactsForCompany, getSocialForCompany, getTechForCompany, Contact } from "@/lib/repo";
...
function findContact(contacts: Contact[], match: (d: string) => boolean) {
  return contacts.find((c) => match((c.designation || "").toLowerCase()));
}

export async function GET() {
  const companies = await listCompanies({ limit: 5000 });
  ...
  const rows = await Promise.all(
    companies.map(async (c) => {
      const contacts = await getContactsForCompany(c.id);
      const social = await getSocialForCompany(c.id);
      const tech = await getTechForCompany(c.id);
      ...
      const techSummary = tech ? [tech.cloud, tech.crm, tech.erp].filter(Boolean).join(" / ") : "";
