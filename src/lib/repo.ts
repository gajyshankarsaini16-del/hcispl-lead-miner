...
export function createUser(input: { name: string; email: string; passwordHash: string; role?: string }) {
  const stmt = db.prepare(
    "INSERT INTO users (name, email, password_hash, role) VALUES (@name, @email, @passwordHash, @role)"
  );
  const info = stmt.run({ ...input, role: input.role ?? "member" });
...
export function getCompanyStats() {
  ...
  const avgLead = db.prepare("SELECT AVG(lead_score) as a FROM companies WHERE status = 'complete'").get() as {
    a: number | null;
  };
...
export function createCompany(input: Partial<Company> & { name: string; created_by?: number }) {
  const stmt = db.prepare(`
    ...
    VALUES (@name, @website, @industry, @status, @created_by)
  `);
  const info = stmt.run({
    name: input.name,
...
export function updateCompany(id: number, fields: Partial<Company>) {
  ...
  db.prepare(`UPDATE companies SET ${assignments}, updated_at = datetime('now') WHERE id = @id`).run({
    ...fields,
    id,
  });
...
export function addContact(input: Omit<Contact, "id">) {
  const stmt = db.prepare(`
    ...
    VALUES (@company_id, @name, @designation, @department, @business_email, @business_phone, @linkedin, @confidence_score, @source)
  `);
  stmt.run(input);
}

export function getSocialForCompany(companyId: number): SocialProfile | undefined {
  return db.prepare("SELECT * FROM social_profiles WHERE company_id = ?").get(companyId) as
    | SocialProfile
    | undefined;
}
...
export function upsertSocial(input: SocialProfile) {
  db.prepare(`
    ...
    VALUES (@company_id, @linkedin, @facebook, @instagram, @x, @youtube)
...
export function getTechForCompany(companyId: number): Technologies | undefined {
  return db.prepare("SELECT * FROM technologies WHERE company_id = ?").get(companyId) as
    | Technologies
    | undefined;
}
...
export function upsertTech(input: Technologies) {
  db.prepare(`
    ...
    VALUES (@company_id, @firewall, @cloud, @email, @erp, @crm, @hosting, @analytics, @cdn, @cms)
    ON CONFLICT(company_id) DO UPDATE SET
      firewall=excluded.firewall, cloud=excluded.cloud, email=excluded.email, erp=excluded.erp,
      crm=excluded.crm, hosting=excluded.hosting, analytics=excluded.analytics, cdn=excluded.cdn, cms=excluded.cms
  `).run(input);
}
...
export function getScoreDistribution(): { high: number; mid: number; low: number } {
  const row = db
    .prepare(
      `SELECT
         SUM(CASE WHEN lead_score >= 70 THEN 1 ELSE 0 END) as high,
         SUM(CASE WHEN lead_score >= 40 AND lead_score < 70 THEN 1 ELSE 0 END) as mid,
         SUM(CASE WHEN lead_score < 40 THEN 1 ELSE 0 END) as low
       FROM companies WHERE status = 'complete'`
    )
    .get() as { high: number | null; mid: number | null; low: number | null };
  return { high: row.high ?? 0, mid: row.mid ?? 0, low: row.low ?? 0 };
}

export function listAllUsers(): Omit<User, "password_hash">[] {
  return db.prepare("SELECT id, name, email, role, created_at FROM users ORDER BY created_at ASC").all() as Omit<
    User,
    "password_hash"
  >[];
}
